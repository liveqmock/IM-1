...
public class BulkLoad {
	...
	public static void main(String[] args) {
		if (args.length == 0) {
			System.out
					.println("usage: java bulkload.BulkLoad <list of ticker symbols>");
			return;
		}
		...
		for (String ticker : args) {
			...
			try (BufferedReader reader = new BufferedReader(
				...
				if (conn.getResponseCode() != HttpURLConnection.HTTP_OK) {
					System.out.println("Historical data not found for "
							+ ticker);
					continue;
				}
				...
				while ((line = csvReader.read()) != null) {
					...
					writer.addRow(ticker, DATE_FORMAT.parse(line.get(0)),
